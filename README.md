...
- *listening-intrinsically-hard-at-4B* — **supported**: zero hits again; decomposition failed on the listening side in both directions.
...
3. **n = 5 per campaign is a pilot.** No systematic claims are made or implied. Scaling runs per condition is roadmap item #3.
...
- ~~**Role swap**~~ — done (Campaign 2): a verified one-sided grammar appears in *both* directions, so the bottleneck is listening, not a property of either model alone. Same-family baselines (Gemma×Gemma, Qwen×Qwen) remain.
